FROM dylanfoster/alpine:3
MAINTAINER Dylan Foster <dylan947@gmail.com>

# gpg keys listed at https://github.com/nodejs/node
RUN set -ex \
  && for key in \
    9554F04D7259F04124DE6B476D5A82AC7E37093B \
    94AE36675C464D64BAFA68DD7434390BDBE9B9C5 \
    0034A06D9D9B0064CE8ADF6BF1747F4AD2306D93 \
    FD3A5288F042B6850C66B31F09FE44734EB7990E \
    71DCFD284A79C3B38668286BC97EC7A07EDE3FC1 \
    DD8F2338BAE7501E3DD5AC78C273792F7D83545D \
    B9AE9905FFD7803F25714661B63B535A4C206CA9 \
    C4F0DFFF4E8C1A8236409D08E73BC641CC11F4C8 \
  ; do \
    gpg --keyserver ha.pool.sks-keyservers.net --recv-keys "$key"; \
  done

ENV NPM_CONFIG_LOGLEVEL info
ENV NODE_VERSION 5.11.0

RUN apk add --no-cache binutils-gold curl g++ gcc libgcc libstdc++ linux-headers make paxctl python \
 && curl -sfSL https://nodejs.org/dist/v${NODE_VERSION}/node-v${NODE_VERSION}.tar.xz \
         -o "node-v${NODE_VERSION}.tar.xz" \
 && curl -sfSL "https://nodejs.org/dist/v$NODE_VERSION/SHASUMS256.txt.asc" \
         -o "SHASUMS256.txt.asc" \
 && gpg --verify SHASUMS256.txt.asc \
 && grep " node-v$NODE_VERSION.tar.xz\$" SHASUMS256.txt.asc | sha256sum -c - \
 && tar -xJf "node-v$NODE_VERSION.tar.xz" \
 && cd /node-v${NODE_VERSION} \
 && ./configure --prefix=/usr --without-snapshot \
 && make -j$(grep -c ^processor /proc/cpuinfo 2>/dev/null || 1) \
 && make install \
 && paxctl -cm /usr/bin/node \
 && apk del binutils-gold curl g++ gcc linux-headers make paxctl python \
 && rm -rf \
        /node-v${NODE_VERSION} \
        /node-v${NODE_VERSION}.tar.xz \
        /SHASUMS256.txt.asc \
        /tmp/* \
        /usr/share/man

RUN mkdir -p /data/app

WORKDIR /data/app
CMD ["node"]
